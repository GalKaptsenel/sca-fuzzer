...
struct measurement_method {
	enum Templates type;
	measurement_template wrapper; // wrapper returns an address prior to TEMPLATE_ENTER
};
...
/* MACRO: MEASUREMENT_METHOD:
 ...
 The if statemtent inside the boilerplate code meant to garantee that the compiler will continue to generate the rest of
 the code it gets from the preprocessor.
 It works because the compiler cannot garantee that the statement will allways evaluate to True, and therefore
 he will continue to generate the rest of the function code. On the other hand, we can assert that it will allways be True.
 First Get the current address of the instruction, and then return the obtained address.*/
...
int load_template(size_t tc_size)
{
	...
	for (;	TEMPLATE_ENTER != ((u64*)(template_ptr))[template_pos];
			++template_pos) {
	...
	for (;	TEMPLATE_INSERT_TC != ((u64*)(template_ptr))[template_pos];
	...
	for (;	TEMPLATE_RETURN != ((u64*)(template_ptr))[template_pos];
			++template_pos, ++code_pos) {
	    ...
	    if (TEMPLATE_INSERT_TC == ((u64*)(template_ptr))[template_pos]) {
	        return -3;
	    }
