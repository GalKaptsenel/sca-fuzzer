...
class NumpyRandomInputGenerator(InputGenerator):
    ...
    def _generate_one(self, state: int) -> Tuple[Input, int]:
        ...
        for i in range(len(input_)):
            ...
            data = rng.integers(0, self.max_input_value, size=size, dtype=np.uint64)  # type: ignore
    ...
    def extend_equivalence_classes(self, inputs: List[Input],
        ...
        for i, input_ in enumerate(inputs):
            ...
            taint_u8 = taints[i].view(np.uint8)
            old_u8 = input_.view(np.uint8)
            new_u8 = new_input.view(np.uint8)
            np.copyto(new_u8, old_u8, where=taint_u8.astype(bool))
