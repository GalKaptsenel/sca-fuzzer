...
import sys
import shlex
from utils import run_command, PROJECT_ROOT, INCLUDE_DIR, THIRD_PARTY_DIR, BUILD_DIR
...
def convert_binary_data_to_object(binary_file, symbol_base, object_file):
    tmp_file = shlex.quote(symbol_base)
    bin_file = shlex.quote(binary_file)
    obj_file = shlex.quote(object_file)
    run_command(f"cp {bin_file} {tmp_file}")
    run_command(
        f"{OBJCOPY} -I binary -O elf64-littleaarch64 -B aarch64 "
        f"--rename-section .data=.data,alloc,load,data,contents {tmp_file} {obj_file}"
    )
    run_command(f"rm {tmp_file}")
...
"""
    ...
    ret
    """
    filtered = [l for l in content if '.section' not in l]
    new_content = [pre_lines] + filtered + [post_lines]
...
def assemble_testcase(filename, output_file):
    ...
    run_command(f'{AS} -march=armv9-a+sve+memtag {shlex.quote(modified)} -o {shlex.quote(output_file)}')
    run_command(f'rm {shlex.quote(modified)}')

def compile_and_link(c_file, testcase_file, data_file, output_file):
    run_command(
        f"{CC} -I{INCLUDE_DIR} -I{THIRD_PARTY_DIR} "
        f"-march=armv9-a+memtag+sve -Wall -g -static "
        f"-o {shlex.quote(output_file)} {shlex.quote(c_file)} {shlex.quote(testcase_file)} {shlex.quote(data_file)}"
    )
...
def main():
    ...
    test_case_object = os.path.join(BUILD_DIR, "testcase.o")
    data_file = sys.argv[1]
    data_object_file = os.path.join(BUILD_DIR, "data.o")
    c_file = os.path.join(BUILD_DIR, "loader_aarch64.c")
    output_file = os.path.join(BUILD_DIR, "program")
