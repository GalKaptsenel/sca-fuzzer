...
#
#    def run(self):
#        file_name = "ISA_A64_xml_A_profile-2025-09_ASL1"
...
def generate_logical_immediates(bits=64):
    values = set()
...
class ExplanationParser:
    ...
    def parse(
        ...
        for expl in explanations.findall(".//explanation"):
            enclist = expl.get("enclist", "")

            if variant not in enclist:
                continue

            if not any(variable in (sym.text or "") for sym in expl.findall(".//symbol")):
                continue

            account = expl.find(".//account")
            table = expl.find(".//table")

            if account is not None:
                return ExplanationParser._parse_account(account, ctx, variable)

            if table is not None:
                return ExplanationParser._parse_table(table, variable)

        raise RuntimeError(f"Failed parsing operand {variable}")

    @staticmethod
    def _parse_table(table: ET.Element, variable: str) -> OperandSpec:
        import pdb; pdb.set_trace()
        entries = table.findall('.//entry[@class="symbol"]')
        values = [e.text for e in entries if e.text != "RESERVED"]
    ...
    @staticmethod
    def _parse_account(account: ET.Element, ctx: ParseContext, variable: str) -> OperandSpec:
        import pdb; pdb.set_trace()
        text = "".join(account.itertext())

        width_match = re.search(r"(\d+)-bit", text)
        width = int(width_match.group(1)) if width_match else 0

        # Simplified (extend as needed)
        if "general-purpose" in text:
            from .aarch64_target_desc import Aarch64TargetDesc
            values = Aarch64TargetDesc.registers.get(width, [])
            return OperandSpec(variable, "REG", values, True, False, width)

        if "bitmask" in text:
            values = [str(v) for v in generate_logical_immediates(ctx.data_size_hint or 64)]
            return OperandSpec(variable, "IMM", values, True, False, width, signed=False)

        if "label" in text:
            return OperandSpec(variable, "LABEL", [], True, False, width)

        raise RuntimeError(f"Unsupported account parsing: {variable}")
...
class TemplateParser:
    ...
    @staticmethod
    def parse(template: str, instr: InstructionSpec, explanations, variant):
        return TemplateParser._parse_inner(template, instr, explanations, variant, 0, ParseContext())
    ...
    @staticmethod
    def _parse_inner(template, instr, explanations, variant, index, ctx):
        ...
        if ch not in TemplateParser.SPECIAL:
            instr.template += ch
            return TemplateParser._parse_inner(template, instr, explanations, variant, index + 1, ctx)

        if ch == "<":
            end = TemplateParser.find_closing_bracket(template, index)
            var = template[index + 1:end]

            operand = ExplanationParser.parse(instr.name, explanations, variant, var, ctx)
            instr.operands.append(operand)
            instr.template += f"{{{var}}}"

            ctx.data_size_hint = operand.width

            return TemplateParser._parse_inner(template, instr, explanations, variant, end + 1, ctx)

        if ch == "{":
            import pdb; pdb.set_trace()
            end = TemplateParser.find_closing_bracket(template, index)

            t1 = template[:index] + template[index + 1:end] + template[end + 1:]
            t2 = template[:index] + template[end + 1:]

            return (
                TemplateParser._parse_inner(t1, copy.deepcopy(instr), explanations, variant, index, ctx)
                + TemplateParser._parse_inner(t2, copy.deepcopy(instr), explanations, variant, index, ctx)
            )

        if ch == "(":
            end = TemplateParser.find_closing_bracket(template, index)
            options = template[index + 1:end].split("|")

            for opt in options:
                new_t = template[:index] + opt + template[end + 1:]
                results += TemplateParser._parse_inner(new_t, copy.deepcopy(instr), explanations, variant, index, ctx)
        ...
        raise RuntimeError(f"Unexpected char: {ch}")
...
class Aarch64Transformer:
    ...
    def parse(self):
        for node in self.tree.iter("instructionsection"):
            if node.attrib.get("type") != "instruction":
                continue

            explanations = node.find(".//explanations")
            if explanations is None:
                continue

            self.instructions.extend(self._parse_instruction(node, explanations))

    def _parse_instruction(self, node, explanations):

        results = []

        for variant in node.findall("classes/iclass/encoding"):
            instr = InstructionSpec(name="")

            asm = variant.findall(".//asmtemplate/*")
            if asm is None:
                continue
            asm = "".join(e.text for e in asm)

            instr.name = asm.split()[0]
...
class Downloader:
    ...
    file_name = "ISA_A64_xml_A_profile-2025-09_ASL1"
    URL = f"https://developer.arm.com/-/cdn-downloads/permalink/Exploration-Tools-A64-ISA/ISA_A64/{file_name}.tar.gz"
    ...
    def run(self):
        ...
        subprocess.run(["tar", "xf", f"{self.file_name}.tar.gz", "-C", "tmp"], check=True)

        files = glob.glob("tmp/**/*.xml", recursive=True)
        ...
        with open(self.out_file, "w") as f:
            import json
            json.dump([i.to_dict() for i in transformer.instructions], f, indent=2)
