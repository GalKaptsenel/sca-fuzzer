...
Sequence A = all-genuine, Sequence B = the toggled lane):
  ...
  1. Own-slot indistinguishability (the "noncache" premise, def:xio(a)): the leaker's OWN htrace at its
     own position lo must be the SAME for its genuine and decoy variants -- the leak is NOT in the
  ...
  2. Leaker/detector matrix at the detector (position hi): fix the prefix, then cross the leaker seal
     ...
     Sets that are the same across all four cells are the detector's own footprint (architectural), not
     the leak.
...
def main():
    ...
    if lo == hi:
        ...
        print(f"# mode={CONF.executor_mode} pin={getattr(CONF,'executor_pinned_core',0)} reps={a.reps}\n")
        fr = {}
    ...
    prefix = [inp(seqA[p]) for p in range(lo)]                 # identical context for every measurement
    ...
    # whether the same canonical branch also evicts in the leaker's own run (a co-located self-dependent
    # violation) or whether the leaker is a pure BTB trainer.
    own = {}
    ...
    print(f"## Leaker self-leak (informational): own-slot htrace at position {lo}, genuine vs decoy")
    ...
    print("   -> " + (f"the leaker ALSO self-leaks at sets {own_spread} (co-located self-dependent leak)"
                      if own_spread else "the leaker's own slot is identical -> a PURE cross-input trainer")
          + "\n")

    # --- Steps 1-4: leaker x detector matrix at the detector (position hi) --------------------------
    # The noncache premise (why standard priming misses this) is read straight off this matrix: the leak
    # set is INDEPENDENT of the detector's OWN seal (the two detector cells agree for each leaker), i.e.
    # the DETECTING pair is own-slot-indistinguishable -- priming, which swaps only the detector, sees
    # nothing. The leak instead tracks the LEAKER's seal.
    ...
    print(f"   {'leaker':<9}{'detector':<10}{hdr[3:]}")
    for (lk_name, dt_name), fr in cells.items():
        row = "".join(f"{fr.get(b,0)*100:5.0f}%" + " " * (max(0, 8 - 6)) for b in sorted(allsets))
        print(f"   {lk_name:<9}{dt_name:<10}" + "".join(f"{fr.get(b,0)*100:5.0f}% " for b in sorted(allsets)))

    # A leak set: present (>threshold) with genuine leaker for BOTH detectors, absent with decoy leaker.
