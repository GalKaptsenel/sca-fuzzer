...
Verifies:
  ...
  2. AuthInstructionSpec.generate enforces value_reg != ctx_reg for all AUT* specs.
  3. seal_test_case never produces committed_insts with value_reg == ctx_reg.
...
from src.aarch64.aarch64_pac import PACInstrumentation, AuthInstructionSpec
...
class TestInstructionSpecGenerate(unittest.TestCase):
    ...
    def test_auth_generate_never_produces_rd_eq_rn(self):
        """AuthInstructionSpec.generate never yields value_reg == ctx_reg."""
            ...
            for _ in range(30):
                ...
                with self.subTest(spec=spec.name):
                    self.assertNotEqual(r0, r1,
                        f"{spec.name}: value_reg={r0} == ctx_reg={r1}")
    ...
    def test_stage1_committed_insts_never_have_rd_eq_rn(self):
        """seal_test_case committed_insts never have value_reg == ctx_reg."""
        norm = self.gen.target_desc.reg_normalized
        found_two_reg = False
        for seed in range(200):
            ...
            for fp in fix_points:
                ...
                with self.subTest(seed=seed, mnemonic=ci.name):
                    self.assertNotEqual(r0, r1,
                        f"seed={seed} {ci.name}: value_reg={r0} == ctx_reg={r1}")
